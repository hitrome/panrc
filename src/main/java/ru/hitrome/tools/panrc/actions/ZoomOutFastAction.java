 ...
 */
public class ZoomOutFastAction extends AbstractActionWithCallback {
    
    private static final Logger LOGGER = Logger.getLogger(ZoomOutFastAction.class.getName());
    
    private final ApplicationContext applicationContext;
    
    public ZoomOutFastAction(ApplicationContext applicationContext) {
        super(LanguageUtil.get(ActionConstants.ZOOM_OUT_FAST_ACTION));
        this.applicationContext = applicationContext;
        ImageIcon actionImage = null;
        
        try {
            actionImage = new ImageIcon(ImageIO.read(getClass()
                    .getResourceAsStream(Constants.IMG_CAM_ZOOM_OUT_FAST))
    ...
    private void sendZoomOutFastCommand() {
        ...
        if (applicationContext.getCamQueue() != null) {
            ...
            applicationContext.getCamQueue().put(new QueueSimpleElement(() -> rcc.zoomOutFast(), null));
            
        } else {
            this.actionResult = rcc.zoomOutFast();
        }
    }
